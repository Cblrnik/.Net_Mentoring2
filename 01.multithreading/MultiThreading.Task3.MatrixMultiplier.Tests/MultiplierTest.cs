...
namespace MultiThreading.Task3.MatrixMultiplier.Tests
{
    ...
    public class MultiplierTest
    {
        ...
        {
            var m1 = CreateMatrix(100);

            var m2 = CreateMatrix(100);
            ...
            var secondWatcher = new Stopwatch();
            var syncMultiplier = new MatricesMultiplier();
            secondWatcher.Start();
            syncMultiplier.Multiply(m1, m2);
            secondWatcher.Stop();

            Assert.IsTrue(watcher.ElapsedTicks < secondWatcher.ElapsedTicks);
        }
        ...
        private static Matrix CreateMatrix(int count)
        {
            var matrix = new Matrix(count, count);
